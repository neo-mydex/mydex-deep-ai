...
from src.tools.user._alchemy_client import (
    ALCHEMY_API_KEY,
    ALCHEMY_DATA_API_BASE,
    DEFAULT_TIMEOUT_SECONDS,
    post_json,
)
from src.tools.user._network import NATIVE_TOKEN_METADATA, normalize_network
...
def get_wallet_portfolio(
    ...
    try:
        assets, page_key = fetch_wallet_assets(address, normalized_networks)
        ...
        # 过滤和排序
        filtered_assets = filter_and_sort_assets(assets, min_value_usd)

        # 按网络汇总
        breakdown = build_network_breakdown(filtered_assets)
...
def fetch_wallet_assets(
    ...
    while True:
        payload = build_request_body(address, networks, page_key)
...
def asset_key(item: dict[str, Any]) -> tuple[str, str]:
    return (
...
def map_asset(item: dict[str, Any]) -> dict[str, Any]:
    """将 Alchemy 资产映射为标准格式"""
    ...
    if item.get("tokenAddress") is None:
        metadata = merge_native_metadata_defaults(metadata, NATIVE_TOKEN_METADATA.get(network, {}))
    ...
    price_value = decimal_or_none(usd_price_data.get("value") if usd_price_data else None)
    value_usd = None
...
def build_network_breakdown(assets: list[dict[str, Any]]) -> dict[str, Any]:
    """按网络汇总资产"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="查询钱包链上资产")
    ...
    parser.add_argument("--min-value", type=float, default=0.01, help="最小 USD 值过滤")
