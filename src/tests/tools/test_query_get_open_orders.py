...
from src.tools.perp.get_open_orders import (
    get_open_orders,
...
class TestGetOpenOrders:
    ...
    @patch("src.tools.perp.get_open_orders._build_info")
    def test_returns_list(self, mock_build_info):
        """测试返回订单列表"""
        mock_info = MagicMock()
        mock_info.open_orders.return_value = [
            {"oid": 123, "coin": "BTC", "side": "A", "sz": "0.01"},
        ]
        mock_build_info.return_value = mock_info

        result = get_open_orders(address="0x123", network="mainnet")
...
class TestGetFrontendOpenOrders:
    """get_frontend_open_orders 测试"""
    ...
    def test_returns_list(self, mock_build_info):
        ...
        mock_info.frontend_open_orders.return_value = [
            {"oid": 456, "coin": "ETH", "side": "B", "isPositionTpsl": False},
        ]
        mock_build_info.return_value = mock_info

        result = get_frontend_open_orders(address="0x456", network="mainnet")

        assert isinstance(result, list)
        assert len(result) == 1

    @patch("src.tools.perp.get_open_orders._build_info")
    def test_returns_empty_on_error(self, mock_build_info):
        """测试出错时返回空列表"""
        mock_info = MagicMock()
        mock_info.frontend_open_orders.return_value = "error"
        mock_build_info.return_value = mock_info

        result = get_frontend_open_orders(address="0x789", network="mainnet")
